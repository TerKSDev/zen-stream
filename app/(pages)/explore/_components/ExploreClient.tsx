...
export default function ExploreClient({
   ...
   useEffect(() => {
      ...
      const fetchFilteredData = async () => {
         ...
         try {
            ...
            const data = (await res.json()) as JikanListResponse<AnimeCard>;
   ...
   }, [hasMore, loading, page, searchQuery, selectedGenre, selectedStatus]);
   ...
   return (
      <div className="px-6 md:px-8 lg:px-12 pb-24 flex flex-col gap-6">
         ...
         {loading && page === 1 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6 auto-rows-max">
               {animeList.map((anime, index) => {
                  ...
                  const glowStyle = isTop3
                     ? index === 0
                        ? 'group-hover:shadow-[0_0_30px_rgba(234,179,8,0.4)] ring-1 ring-yellow-500/50'
                        : 'group-hover:shadow-[0_15px_30px_rgba(255,255,255,0.2)] border border-white/20'
                     : 'group-hover:shadow-[0_15px_30px_rgba(160,124,254,0.25)] border border-white/10';

                  return (
                     <Link
                        href={`/anime/${anime.mal_id}`}
                        key={`${anime.mal_id}-${index}`}
                        className="group relative flex flex-col gap-2 rounded-xl overflow-hidden transition-all duration-500 hover:-translate-y-2"
                     >
                        <div
                           className={`relative w-full aspect-3/4 rounded-xl overflow-hidden bg-white/5 transition-all duration-500 ${glowStyle}`}
                        >
                           <Image
                           ...
                           {isTop3 ? (
                              <div
                                 className={`absolute top-2.5 left-2.5 flex items-center gap-1 ${badgeColor} px-2.5 py-1 rounded-lg border shadow-lg`}
                              >
                              ...
                              anime.score && (
                                 <div className="absolute top-2.5 left-2.5 flex items-center gap-1 bg-black/60 backdrop-blur-md px-2 py-1 rounded-lg border border-white/10 shadow-lg">
                                    <IoStar
                                       className="text-yellow-400 drop-shadow-[0_0_5px_rgba(250,204,21,0.8)]"
                                       size={12}
                                    />
                           ...
                           <div className="absolute bottom-0 left-0 w-full p-4 transform translate-y-3 group-hover:translate-y-0 transition-transform duration-300">
                              <h3 className="text-sm font-bold text-white line-clamp-2 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] leading-snug">
                              ...
                              <div className="flex flex-wrap gap-1.5 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">
                                 <span className="text-[9px] font-black px-1.5 py-0.5 bg-anime-primary/80 rounded text-white uppercase tracking-wider">
                                    {anime.type}
                                 </span>
                                 {anime.genres
                                    ?.slice(0, 2)
                                    .map((genre: AnimeGenre) => (
                                       <span
                                          key={genre.mal_id}
                                          className="text-[9px] font-bold px-1.5 py-0.5 bg-white/20 backdrop-blur-sm rounded text-white/90 whitespace-nowrap"
                                       >
                                          {genre.name}
                                       </span>
                                    ))}
         ...
         {hasMore && animeList.length > 0 && (
            <div
               ref={lastElementRef}
               className="w-full flex justify-center mt-8 mb-4 h-10"
            >
               {loading && page > 1 && (
         ...
         {animeList.length === 0 && !loading && (
            <div className="w-full py-24 md:py-32 flex flex-col items-center justify-center gap-6 text-center px-4">
               ...
               <div className="relative w-48 h-48 sm:w-64 sm:h-64 mb-2 drop-shadow-[0_0_40px_rgba(160,124,254,0.15)] group">
                  ...
                  */}
                  <div className="absolute inset-0 bg-linear-to-br from-anime-primary/20 to-transparent rounded-full animate-pulse" />
                  <div className="absolute inset-4 bg-[#0B0E14] rounded-full flex items-center justify-center border border-white/5 shadow-[inset_0_0_50px_rgba(0,0,0,0.8)]">
                     <IoSearch
                        size={72}
                        className="text-slate-600/30 group-hover:scale-110 transition-transform duration-500"
                     />
                     <IoCloseCircle
                        size={36}
                        className="text-anime-primary absolute bottom-10 right-10 md:bottom-14 md:right-14 drop-shadow-[0_0_15px_rgba(160,124,254,0.8)]"
                     />
                  </div>
               </div>
               ...
               <p className="text-sm md:text-base text-slate-400 max-w-md leading-relaxed">
                  We couldn&apos;t find any anime matching your exact filters.
                  Try tweaking your search or exploring a different genre.
               </p>
